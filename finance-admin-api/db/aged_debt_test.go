...
func (suite *IntegrationSuite) Test_aged_debt() {
	...
	_, invRef := suite.seeder.CreateInvoice(ctx, client1ID, fh.InvoiceTypeAD, nil, &date, nil, nil, nil)
	paidInvoiceID, _ := suite.seeder.CreateInvoice(ctx, client1ID, fh.InvoiceTypeAD, nil, &date, nil, nil, nil)
	writeOffID := suite.seeder.CreateAdjustment(ctx, client1ID, paidInvoiceID, fh.AdjustmentTypeWriteOff, 100, "Written off")
	...
	assert.Equal(suite.T(), "Yes", results[0]["Active case?"], "Active case?")
	...
	assert.Equal(suite.T(), invRef, results[0]["Trx number"], "Trx number")
