...
func (suite *IntegrationSuite) Test_aged_debt() {
	...
	_, c1i1Ref := suite.seeder.CreateInvoice(ctx, client1ID, fh.InvoiceTypeAD, nil, &d2024, nil, nil, nil)
	paidInvoiceID, _ := suite.seeder.CreateInvoice(ctx, client1ID, fh.InvoiceTypeAD, nil, &d2024, nil, nil, nil)
	...
	suite.seeder.CreateFeeReduction(ctx, client2ID, fh.FeeReductionTypeRemission, "2019", 2, "A reduction")
	_, c2i1Ref := suite.seeder.CreateInvoice(ctx, client2ID, fh.InvoiceTypeAD, nil, &d2020, nil, nil, nil)
	_, c2i2Ref := suite.seeder.CreateInvoice(ctx, client2ID, fh.InvoiceTypeS2, &general, &d2022, &d2022, nil, nil)
	...
	assert.Equal(suite.T(), "10482009", results[0]["Revenue cost centre"], "Revenue cost centre - client 1")
	assert.Equal(suite.T(), "Supervision Investigations", results[0]["Revenue cost centre description"], "Revenue cost centre description - client 1")
	assert.Equal(suite.T(), "4481102093", results[0]["Revenue account code"], "Revenue account code - client 1")
	assert.Equal(suite.T(), "INC - RECEIPT OF FEES AND CHARGES - Appoint Deputy", results[0]["Revenue account code description"], "Revenue account code description - client 1")
	assert.Equal(suite.T(), "AD", results[0]["Invoice type"], "Invoice type - client 1")
	assert.Equal(suite.T(), c1i1Ref, results[0]["Trx number"], "Trx number - client 1")
	assert.Equal(suite.T(), "AD - Assessment deputy invoice", results[0]["Transaction Description"], "Transaction Description - client 1")
	...
	assert.Equal(suite.T(), "100.00", results[0]["Original amount"], "Original amount - client 1")
	...
	assert.Equal(suite.T(), "100.00", results[0]["0-1 years"], "0-1 years - client 1")
	...
	assert.Equal(suite.T(), "2020-01-01", results[1]["Invoice date"], "Invoice date - client 2, invoice 1")
	assert.Equal(suite.T(), "2020-01-31", results[1]["Due date"], "Due date - client 2, invoice 1")
	assert.Equal(suite.T(), "2019/20", results[1]["Financial year"], "Financial year - client 2, invoice 1")
	...
	assert.Equal(suite.T(), "2022-01-01", results[2]["Invoice date"], "Invoice date - client 2, invoice 2")
	assert.Equal(suite.T(), "2022-01-31", results[2]["Due date"], "Due date - client 2, invoice 2")
	assert.Equal(suite.T(), "2021/22", results[2]["Financial year"], "Financial year - client 2, invoice 2")
	...
	assert.Equal(suite.T(), "0", results[2]["1-2 years"], "1-2 years - client 2, invoice 2")
	assert.Equal(suite.T(), "320.00", results[2]["2-3 years"], "2-3 years - client 2, invoice 2")
